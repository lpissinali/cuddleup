...
/*
    ...
    $('#imagesForm').bind('fileuploadalways', function (e, data) {
        picturesLeft();
        bindGalleryEvents();
    });

    $('#imagesForm').bind('fileuploaddestroyed', function (e, data) {
        updateGalleryTabs();
        picturesLeft();
    });
    ...
    // Load existing files:
    $('#imagesForm').addClass('fileupload-processing');
    $.ajax({
        // Uncomment the following to send cross-domain cookies:
        //xhrFields: {withCredentials: true},
        url: $('#imagesForm').fileupload('option', 'url'),
        dataType: 'json',
        context: $('#imagesForm')
    }).always(function () {
        ...
        //profile tabs
        if ($('#private').length>0) {
            $('.template-download.private').hide();
    ...
    $('#imagesForm').bind('fileuploadstart', function (e, data) {
        $('#alert-noimages').fadeOut(500);
    });



    //gallery
    $('#edit-pic').on('hidden.bs.modal', function(e) {
        picturesLeft();
        updateGalleryTabs();
    });
    $('#gallery-next').click(function(e) {
        e.preventDefault();
        currentImage = currentImage.parent().next().find('a').first();
        updateGallery();
    });

    $('#gallery-prev').click(function(e) {
        e.preventDefault();
        currentImage = currentImage.parent().prev().find('a').first();
        updateGallery();
    });

    $('#gallery-make-default').click(function(e) {
        e.preventDefault();
        e.stopPropagation();
        var url = currentImage.parent().find('a.gallery-main').attr('href');

        $.ajax(url, {success: function() {
            //in image list:
            $('i.gallery-default').addClass('hide');
            currentImage.find('i.gallery-default').removeClass('hide');
            //in gallery:
            //$('#gallery-default').removeClass('hide');
            //$('#gallery-make-default').addClass('hide');
            updateGallery();
        }});

    });

    $('#gallery-lock-selector input').change(function(e) {
        if ($(this).val()==1) { //lock
            var action = 'lock';
        } else {
            var action = 'unlock';
        ...
        var url = currentImage.parent().find('a.gallery-' + action).attr('href');

        $.ajax(url, {success: function() {
            //in image list:
            if (action=='lock') {
                currentImage.find('i.gallery-locked').removeClass('hide');
                currentImage.parent().addClass('private');
            } else {
                currentImage.find('i.gallery-locked').addClass('hide');
                currentImage.parent().removeClass('private');
            }
    ...
    $('#gallery-delete').click(function(e) {
        e.preventDefault();
        currentImage.parent().find('a.gallery-delete').click();
    ...
    $('#gallery-rotate-right').click(function(e) {
        e.preventDefault();
        rotate('right');
    });

    $('#gallery-rotate-left').click(function(e) {
        e.preventDefault();
        rotate('left');
    });
...
function rotate(dir) {
    var url = currentImage.parent().find('a.gallery-rot-' + dir).attr('href');

    $('#gallery-img').attr('src', $('#gallery-preloader').attr('src'));
    $.ajax(url, {dataType: 'json', success: function(resp) {
        //currentImage.data('href', resp.url);
        currentImage.parent().find('.gallery-item').data('href', resp.url);
        currentImage.find('img').attr('src', resp.thumbnails_url);
        updateGallery();
    }});
...
function bindGalleryEvents() {
    $('a.gallery-item').click(function(e) {
        e.preventDefault();
        currentImage = $(this).parentsUntil('.template-download').last().parent().find('.gallery-item').first();
        updateGallery();
    });

    $('.gallery-lock,.gallery-unlock').unbind('click').click(function(e) {
        e.preventDefault();

        var url = $(this).attr('href');
        var _this = $(this);
        $.ajax(url, {dataType: 'json', success: function(resp) {
            var p = _this.parentsUntil('.template-download').last().parent();
            p.toggleClass('private');
            p.find('.gallery-locked').toggleClass('hide');
            p.find('.gallery-lock').toggleClass('hide');
            p.find('.gallery-unlock').toggleClass('hide');
            updateGalleryTabs();
            picturesLeft();
        }});
    });
...
function updateGallery() {
    ...
    $('#gallery-img').attr('src', $('#gallery-preloader').attr('src'));
    $('#gallery-img').attr('src', currentImage.data('href'));

    var par = currentImage.parent();

    if (par.next().find('a').length>0) {
        $('#gallery-next').show();
    } else {
        $('#gallery-next').hide();
    }
    if (par.prev().find('a').length>0) {
        $('#gallery-prev').show();
    } else {
        $('#gallery-prev').hide();
    }

    $('#gallery-total').text(par.siblings().length);
    $('#gallery-current').text(par.prevAll().length+1);

    //LOCKED PICS
    if (par.hasClass('private')) {
        var locked = 1;
    } else {
        var locked = 0;
    }
    var block = $('#gallery-lock-selector');
    block.find('input[value=' + locked + ']').prop('checked', true);
    block.find('label').removeClass('active');
    block.find('label[input_value=' + locked + ']').addClass('active');

    //MAIN/DEFAULT PIC
    if (currentImage.find('i.gallery-default').hasClass('hide')) { //not default
        $('#gallery-default').addClass('hide');
        $('#gallery-make-default').removeClass('hide');
        $('#gallery-lock-selector').show();
        $('#gallery-default-selector').show();
        //var def = 0;
    } else {
        $('#gallery-default').removeClass('hide');
        $('#gallery-make-default').addClass('hide');
        $('#gallery-default-selector').hide();
        //var def = 1;

        //cannot lock default image:
        $('#gallery-lock-selector').hide();
    }

    /*
    //default selector is either hidden or set to 'not default'
    var block = $('#gallery-default-selector');
    block.find('input[value=' + def + ']').prop('checked', true);
    block.find('label').removeClass('active');
    block.find('label[input_value=' + def + ']').addClass('active');
    */


    if (locked==1) {
        $('#gallery-make-default').addClass('hide'); //cannot default locked image
        $('#gallery-default-selector').hide();
    } else {
        //$('#gallery-default-selector').show();
    }
...
function updateGalleryTabs()
{
    if ($('#private').length>0) {
        if ($('#private').is(':visible')) {
            ...
            $('.template-download.private').show();
        } else {
...
function picturesLeft(){
    var maxVal = 12;
    var curVal = $('.template-download').length;

    var _private = $('.template-download .gallery-locked:not(.hide)').length;
    $('#private-count').text("(" + _private + ")");
    $('#public-count').text("(" + (curVal - _private) + ")");

    if ($('.ico-pending').length>0 || $('.ico-pending2').length>0) {
        $('#pending-box').removeClass('hide');
    } else {
        $('#pending-box').addClass('hide');
    }

    $('#remaining-pictures').text(maxVal-curVal);

    if(curVal >= 12) {
        $('#remaining-pictures').text('0');
        $('.add-pic').hide();
        $('#count-box').hide();
        $('#limit-box').removeClass('hide');
    }
    else {
        $('.add-pic').show();
        $('#count-box').show();
        $('#limit-box').addClass('hide');
    }
}